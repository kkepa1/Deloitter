...
public class UserController {
    ...
    public User getUser(@PathVariable String name) {return userService.findUserByName(name);}

    @PostMapping(value = "")
    public User saveUser(@Valid @RequestBody User user) {return userService.saveUser(user);}

    @PutMapping(value = "")
    public User updateUser(@Valid @RequestBody User user) {return userService.updateUser(user);}
